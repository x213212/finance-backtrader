...
def rsi(parmam):
    ...
    #技術指標
    RSI= popmoder.get_RSI(np.array(close))
    ...
    for tmp in range(14,len(RSI)):
        if(tmp+1<len(RSI)):
            ex_handle=(int(price/(close[tmp]*1000))-1)
            if(model == "gen"):
                if(RSI[tmp]<25  and tmp>start and tmp<end ):
                    ...
                    # print(ex_data_list)
                elif (RSI[tmp]>65 and tmp>start and tmp<end  ):
                    data_count_num =  tmp 
                    total_date_count = data_count_num 
                    total_date_str = date[data_count_num]
                    ex_data_list = data_list[data_count_num]
                ...
                if(RSI[tmp]<25  and tmp>start and tmp<end   and ex_handle >=1 and price >= (ex_handle*close[tmp]*1000) ):
                ...
                elif (RSI[tmp]>65 and tmp>start and tmp<end  and re_handle >=1 ):
